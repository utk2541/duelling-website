...
const findproblems = async (
  ...
  let finalproblems : { status : string ; problems : Array<dproblem>} = { status: "OK", problems: Array<dproblem>() };
  ...
  if (finalproblems.problems.length == 0) finalproblems.status = "NOT ENOUGH PROBLEMS IN GIVEN RANGE";
